...
== Überblick
Modbus ist ein Industrieprotokollstandard der Firma Modicon, heute Schneider Electrics, das in den späten 70er Jahren auf dem Markt kam.
...
Der Bereich an Funktionscode reicht von 1-255, wobei 128-255 für Fehlermeldungen reserviert sind. Die Funktionscodes kann man noch unterteilen in öffentliche, User definierte und reservierte Funktionscodes (siehe @mod-kategorien).
...
==== RS485
Die RS485 Schnittstelle weist dieselben elektrischen Eigenschaften wie RS422 auf. Dazu kommt allerdings, dass RS485 *Multipoint-to-Multipoint* fähig ist. In der häufigsten Bauweise, die nur as zwei Drähten besteht ist sie jedoch nur Halbduplex fähig. Die Vollduplex Bauart mit vier  ist hingegen nur sehr selten zu finden.
...
===	Security
In Modbus wird sowohl der Funktionscode als auch die Daten als Plain Text übertragen und es gibt an Security Features. Allerdings kann Modbus TCP/IP mit #htl3r.long[tls] (#htl3r.short[tls]) verwendet werden. Damit werden sowohl Daten Integrität, also auch Verschlüsselung und Authentifizierung bereitgestellt. Das geschieht, indem TLS mithilfe eines Handshakes einen sicheren Tunnel aufbaut, wobei zuerst die Verschlüsselungsverfahren gewählt werden und sich dann die zwei Geräte gegenseitig mit Zertifikaten authentifizieren. Weiters wird ein session-key ausgehandelt, mit dem im weiteren Verlauf die Nachrichten bis zur Schließung der Session ver- und entschlüsselt werden. 