...
Man kann nun testen, ob die Installation erfolgreich war. #footnote[ Die aktuelle Version ist 0.4.7 (Stand 12.12.2024)]
...
===	Host-Discovery
Host-Discovery bezeichnet den Prozess, Informationen über die Geräte in einem Netzwerk zu sammeln. In diesem Fall wird es verwendet, um die IP-Adressen des Raspberry Pi sowie der Siemens LOGO SPS herauszufinden und zu testen, ob diese online sind. Hierfür wird nmap - mit der Option -sn - verwendet, welche einen Ping-Scan durchführt, ohne die Ports schon zu scannen. 

```
C:\Users\esthie>nmap -sn 10.100.0.0/24
Starting Nmap 7.95 ( https://nmap.org ) at 2024-12-11 11:46 Mitteleuropõische Zeit
...
Der Ping-Scan zeigt, dass zum Zeitpunkt des Scans drei Geräte auf den ICMP-Echo-Request antworten. Diese Geräte sind die Siemens LOGO SPS mit 10.100.0.1, der Raspberry Pi mit 10.100.0.2 sowie der Management-PC, von dem aus der Ping-Scan ausgeführt wurde. 
...
=== nmap
Für nmap werden die folgenden beiden Befehle verwendet. 
...
Die Option `-msf` legt den Modus des Scans fest, welcher in diesem Fall (`sf`) wieder ein TCP-SYN-Scan ist. Bei unicornscan gibt es keine Option für alle Ports, weshalb diese manuell eingegeben werden müssen 
(`-p1-65535`).
