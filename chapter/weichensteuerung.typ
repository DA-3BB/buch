...
// Bild von den Gleisen?

Die SPS verarbeitet die Nutzeranfragen - den Befehl zum Umschalten der Weichen - und sendet das Kommando an den Raspberry Pi via Modbus/TCP. Der Raspberry Pi fungiert als Übersetzer, indem er aus den Befehlen der SPS (diese sind Wahrheitswerte - entweder `true` oder `false`) passende Befehle für die Servos (diese arbeiten mit Pulsweitenmodulation) erzeugt.

== Konfiguration der Siemens LOGO! SPS
Die Konfigurationen, welche auf der SPS getätigt werden müssen, sind minimal. Das Programm selbst ist sehr einfach gehalten. Das grundlegende Konzept ist, dass jeder Eingang entweder 0 oder 1 empfängt und das direkt an den Raspberry Pi weiterschickt.

Die Kommunikation der SPS läuft ausschließlich über den Ethernet-Adapter und somit das LAN. Daher können die physischen Ein- und Ausgänge der SPS nicht im Programm verwendet werden. Stattdessen werden Merker als Eingänge und Netzwerkausgänge als Ausgänge verwendet.
...
Auf dem Raspberry Pi befinden sich verschiedene Skripte (Python3 beziehungsweise Bash), welche für die Weichensteuerung entscheidend sind und den Raspberry Pi zum Modbus-Server werden lassen. Diese werden im folgenden Abschnitt einzeln beschrieben.
...
1. `install.sh` erzeugt das Verzeichnis `/opt/3bb/modbus` und kopiert benötigte Skripte in das neu erstellte Verzeichnis.
2. `update.sh` wird verwendet, aktualisierte Skripte in das vorhandene Verzeichnis zu kopieren.
3. `prep_python.sh` bereitet das virtuelle Environment (venv) für Python vor und installiert benötigte Pakete.
...
Die Klasse `modBusDummyServant` dient dazu, die Verbindung zwischen der SPS und dem Raspberry Pi zu testen. Der Server hört hierbei auf Anfragen der SPS, initialisiert dabei aber keine I2C-Treiber der Servos. Wenn der Modbus-Server Pakete der SPS erhält, werden Log-Nachrichten auf der Konsole ausgegeben.
...
Das Skript funktioniert wie das zuvor genannte Skript zum Starten des Servers, jedoch wird die Option `-test` (Zeile 3) beim Aufruf hinzugefügt. Die Servo-Objekte werden nicht initialisiert. Man kann die Kommunikation zwischen der SPS und dem Raspberry Pi nun testen, ohne dass man auf die Servos achten muss.

#pagebreak()

Neben dem Skript zum Testen der Kommunikation zwischen dem Raspberry Pi und der SPS gibt es ein weiteres Skript, welches dazu dient, die Verbindung zu den Servos zu testen. Dieses wurde in Python geschrieben und ähnelt `modbus_server.py`.
...
In Zeile 17 ist ersichtlich, dass der Raspberry Pi eine neue Verbindung zu einem Client - der SPS - aufgebaut hat. Der Server ist also nun funktionsbereit.

#pagebreak()

Man kann nun den Webserver der SPS von einem Management-PC starten. Die Addressierung lautet: `http://10.100.0.1`. Es öffnet sich eine Seite, auf der der Nutzer sich authentifizieren muss. Das Passwort für den User "Web User" ist auf dem Default-Wert ("admin") belassen worden.
...
In den folgenden beiden Bildern befindet sich Screenshots aus dem "LOGO! Variablen"-Tab der SPS. Man beachte, dass Merker 4 in der letzten Zeile vom Wert false auf den Wert true gesetzt wird.
...
Nun wird betrachtet, welche Log-Nachrichten der Raspberry Pi ausgibt. Diese lauten wie folgt:
