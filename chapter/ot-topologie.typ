...
Um einer realitätsnahen Eisenbahnstrecke gerecht zu werden, ist die Eisenbahn in isolierte Blöcke eingeteilt. Dies verhindert, dass an allen Gleisen Strom anliegt. Somit entsteht einerseits kein Kurzschluss durch die Wendeschleifen und andererseits ist die Gefahr minimiert, dass zwei Züge aufeinander auffahren. Damit wird ermöglicht, dass mindestens ein Block Abstand zwischen den Zügen vorhanden ist.
...
Diese Blöcke, aber auch die Weichen, werden durch ein zentrales Steuergerät, im Speziellen durch eine #htl3r.long[sps] (#htl3r.short[sps]), der 'Siemens LOGO!' gesteuert. Dieses sendet Modbus TCP/IP Frames zu sogenannten #htl3r.longpl[rtu] (RTUs) um Daten wie die aktuelle Weichenstellung abzufragen.
\ \
Um die Komponenten in der Nähe der Modelleisenbahnanlage zu plazieren, wurde ein #htl3r.short[ot]-Netzwerkschrank gebaut, der die Zentrale abbildet. In diesem befindet sich einerseits die Stromversorgung, die #htl3r.short[sps] und ein #htl3r.short[ot]-Switch, andererseits auch ein Microcontroller, im Speziellen ein Raspberry PI, der als #htl3r.short[rtu] für die Weichen dient.
...
\ \
Weiters erfasst die Steuerung Reflex-Lichtschranken, um die Einfahrt eines Zuges in einen Block zu erkennen. Auch diese Information werden von der #htl3r.short[sps] über die Relaismodule abgefragt.

=== Stromversorgung
Bei der Stromversorgung wurde darauf geachtet, dass in den zugänglichen Teilen der Anlage keine 230V Netzspannung vorhanden ist, um die Sicherheit beim Arbeiten und im späteren Verlauf, wenn Schüler*innen die Steuerung als Laborübung nutzen, zu gewährleisten.
\
Der Switch benötigt dabei als einziges Gerät eine 24V Gleichspannungsversorgung, der Raspberry PI 5.5V und alle anderen Komponenten - #htl3r.short[sps], Relaismodule, Reflex-Lichtschranken - werden mit einem 12V Netzteil betrieben.
...
Das Herzstück der Steuerung ist die #htl3r.long[sps] beziehungsweise die Siemens LOGO!, da sie als Modbus Client die Steuerung vom Raspberry und den Relais Modulen übernimmt. Sie wird dabei mit der Software "LOGO! Soft Comfort" in der Version 8.4 programmiert. Um ein Schaltprogramm zu schreiben, wird ein externes Gerät mit der Software "LOGO!Soft Comfort" benötigt. Sobald das Programm auf die #htl3r.short[sps] gespielt wird, kann man es von dort über das kleine Display starten und stoppen.
\ \
Zum Programmieren selbst wurde in diesem Projekt die Sprache "#htl3r.long[fup]". "#htl3r.short[fup]" ist eine grafische Programmiersprache, die mit logischen Funktionsblöcken arbeitet. Diese Blöcke sind einerseits logische Gatter wie 'AND' und 'XOR', aber auch verschiedene andere Bausteine wie Timer oder Counter. Der Vorteil von "#htl3r.short[fup]" ist, dass die Sprache einerseit leicht zu lernen ist und andererseits mit einem Verständnis von boolschen Operatoren einfach zu verstehen ist. Allerdings kann bei größeren Projekten die Übersicht leicht verloren gehen und Elemente wie Funktionen oder Bedingungen sind nicht vorhanden.
...
Bevor der Diagram Editor im Network Project benutzt werden kann, muss man sich entweder zuerst mit der #htl3r.short[sps] verbinden oder ein Projekt mit einer #htl3r.short[sps] erstellen. Ein Projekt erstellen man indem man eine SPS mit "Add new Device" hinzufügt und die Einstellungen speichert.
...
=== Raspberry PI
Der Raspberry übernimmt als Modbus Server die Weichensteuerung mittels eines Pythonskript. Wie in @piverkabelung erwähnt, sind die Servomotoren an dem Raspberry PI, genauer gesagt an dem Servo Driver HAT "16 Kanal Servo Driver uHAT (I2C) für Raspberry Pi" angeschlossen. Dieser dient zur Erweiterung der #htl3r.short[pwm] Outputpins, da der Raspberry PI nur eine limitierte Anzahl davon hat. Dadurch, dass der Servo Driver HAT die Stromversorgung von dem Raspberry PI mitverwendet, zittern die Servos. Dieses Problem wurde gelöst, indem am Servo Driver HAT ein Wiederstand abgezwickt wurde, um die Stromversorgung seperat zu legen. \
...
#pagebreak()
=== Relais Modul
Um die Blöcke der Eisenbahnstrecke anzusteuern, werden drei Relaismodule mit Modbus Funktion verwendet. Dabei handelt es sich um das Modbus POE ETH Relay (B) des Unternehmens Waveshare. Es verfügt über 8 Relais Ausgänge, einem Ethernetport und 8 digitalen Eingänge. Die drei Module sollen dabei die dezentrale Steuerung darstellen, die nur noch mithilfe eines Ethernetkabels an den Hauptstandort, die Zentrale, angebunden ist.
...
- 0x0002 toggle Modus: Relaisstatus wird bei einem Impuls am jeweiligen Input umgeschaltet.
- 0x0003 edge Modus: Relaistatus wird bei einem Flankenwechsel am jeweiligen Input umgeschaltet.

=== Reflex-Lichtschranken
Um den Blockübertritt zu erkennen, werden Reflex-Lichtschranken an beiden Seiten der Isolierung eingesetzt, um die Richtung der Modelleisenbahn zu erkennen. Die Reflex-Lichtschranken sind dabei am Input der Relaismodule angeschlossen und die #htl3r.short[sps] kann sie über diese abfragen. \
