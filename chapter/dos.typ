...
Ein Denial of Service-Angriff (#htl3r.short[dos]-Angriff) ist eine Attacke auf die Verfügbarkeit eines Services auf einem Gerät. Hierbei möchte ein Angreifer die
normale Funktionalität eines Services unterbrechen beziehungsweise behindern, indem er die Dienste auf dem Computer mit Anfragen überlädt. Ein #htl3r.short[dos] wird von einem Gerät aus gestartet - werden mehrere verwendet (z.B. aus einem Botnet), dann spricht man von einem #htl3r.short[ddos]-Angriff.

#htl3r.short[dos]-Angriffe sind effektiv - auch gegen starke Ziele. Sie können einfach und ohne großen Aufwand ausgeführt werden. Angreifer überfluten ihr Ziel mit Massen an Paketen, die die Kapazität des Geräts überfordern, welches so neuen Nutzern den Zugang zu Services verweigern und bestehende Verbindungen verlangsamt.

=== Slowloris
Slowloris ist ein Tool, welches speziell für Angriffe auf Webserver geschrieben wurde. Es öffnet eine Vielzahl an #htl3r.short[http]-  bzw. #htl3r.short[https]-Verbindungen und versucht, diese so lange wie möglich offen zu halten. Ein großer Vorteil des Tools ist es, dass es kaum Netzwerkressourcen benötigt.
...
Ab Zeile 9 findet sich das Ergebnis des Scans: "`Probably vulnerable`". Die #htl3r.short[sps] ist also anfällig für einen #htl3r.short[dos]-Angriff.

#pagebreak()

==== Vorbereitung von Slowloris
Slowloris wird in einer Virtuellen Maschine am Management-PC installiert. Das Betriebssystem der #htl3r.short[vm] kann beliebig gewählt werden. Im folgenden wird die Kali-Linux-#htl3r.short[vm] (Version 2024.3) verwendet, welche im Kapitel Port-Scanning für unicornscan aufgesetzt wurde. Hierfür werden die folgenden Befehle ausgeführt.
...
Aus der Abbildung ist ersichtlich, dass die Weiche 2 einen neuen Wert erhält. Die Kommunikation zwischen der #htl3r.short[sps] und dem Raspberry Pi ist also erfolgreich.
...
In diesem Abschnitt des Skripts wird überprüft, ob der Server noch erreichbar ist. Wie auch bei der Methode `send_request` wird eine Verbindung an die gegebene #htl3r.short[url] gesendet und anhand deren Antwort überprüft, ob der Server mit dem Statuscode 200 (OK) antwortet. Der Unterschied zur vorherigen Methode ist, dass die Methode nicht die Anzahl der Erfolge bzw. der Fehlschläge erhöht, sondern stattdessen eine Nachricht auf der Konsole ausgibt. Diese Methode wird alle 100 Anfragen zusätzlich ausgeführt und gibt dem Nutzer so eine Einsicht während der Ausführung des Skripts.
...
=== Fazit
Ein #htl3r.short[dos]-Angriff stellt also eine Gefahr für die #htl3r.short[sps] und damit das gesamte Bahnnetzwerk dar, da der User nicht auf Weichen und Blockschaltung zugreifen kann. Weiters kann man nicht sicher sein, was mit dem Programm oder gespeicherten Variablen während einer Überlastung auf der #htl3r.short[sps] passiert.

Erwähnenswert ist, dass der Webserver der #htl3r.short[sps] eine enorme Menge an Verbindungen ohne Probleme verarbeiten konnte.
